...
var coop = {
    showHtml: '',
    // 确认框
    open_confirm: function(title, content, func) {
        this.showHtml = '<div class="weui-mask"></div>\
            <div class="weui-dialog weui-skin_android">\
                ...
                <div class="weui-dialog__ft">\
                    <a onclick="coop.close(this, \'weui-skin_android\')" class="weui-dialog__btn weui-dialog__btn_default">取消</a>\
                    <a id="coop-confirm-btn" href="javascript:;" class="weui-dialog__btn weui-dialog__btn_primary">确认</a>\
                </div>\
                \
            </div>';
        this.showHtml = this.showHtml.replace(/{title}/g, title);
        this.showHtml = this.showHtml.replace(/{content}/g, content);
        this.show();
        $('#coop-confirm-btn').bind('click', function(){
            coop.close(this, 'weui-skin_android');
            if (func) {
                func();
            }
        });
    ...
    // 输入框
    open_input: function (title, func) {
        this.showHtml = '<div class="weui-mask"></div>\
    <div class="weui-dialog coop-dialog-input">\
        <div class="weui-dialog__hd"><strong class="weui-dialog__title">{title}</strong></div>\
        <div class="weui-dialog__bd">\
            <textarea id="coop-dialog-text" class="weui-textarea" placeholder="请输入文本" rows="4"></textarea>\
        </div>\
        <div class="weui-dialog__ft">\
            <a onclick="coop.close(this, \'coop-dialog-input\')" class="weui-dialog__btn weui-dialog__btn_default">取消</a>\
            <a id="coop-confirm-input" href="javascript:;" class="weui-dialog__btn weui-dialog__btn_primary">确定</a>\
        </div>\
    </div>';
        this.showHtml = this.showHtml.replace(/{title}/g, title);
        // this.showHtml = this.showHtml.replace(/{content}/g, content);
        ...
        $('#coop-confirm-input').bind('click', function(){
            var textcontent = $('#coop-dialog-text').val();
            if (textcontent) {
                coop.close(this, 'coop-dialog-input');
                if (func) {
                    func(textcontent);
                }
            }
    ...
    // alert
    open_alert: function (title, content) {
        this.showHtml = '<div class="weui-mask"></div>\
    <div class="weui-dialog coop-dialog-alert">\
        <div class="weui-dialog__hd"><strong class="weui-dialog__title">{title}</strong></div>\
        <div class="weui-dialog__bd">{content}</div>\
        <div class="weui-dialog__ft">\
            <a onclick="coop.close(this, \'coop-dialog-alert\')" href="javascript:;" class="weui-dialog__btn weui-dialog__btn_primary">确定</a>\
        </div>\
    </div>';
        this.showHtml = this.showHtml.replace(/{title}/g, title);
        this.showHtml = this.showHtml.replace(/{content}/g, content);
    ...
    // 提示
    open_toast: function (title, time) {
        this.showHtml = '<div id="coop-toast-msg" >\
        ...
        setTimeout(function() {
            coop.close('#coop-toast-msg');
        }, time * 1000);
    },
    // loading层
    open_loading: function() {
        this.showHtml = '<div id="coop-toast-loading" >\
        <div class="weui-mask_transparent"></div>\
        <div class="weui-toast">\
        ...
        this.show();
        return '#coop-toast-loading';

    },
    // 显示msg框
    open_msg_page: function(title, content, url, icon) {
        var show_icon = 'weui-icon-success';
        if (icon) {
            show_icon = 'weui-icon-' + icon;
        }
        ...
        this.showHtml = '<div class="weui-msg">\
    <div class="weui-msg__icon-area"><i class="{show_icon} weui-icon_msg"></i></div>\
    <div class="weui-msg__text-area">\
        <h2 class="weui-msg__title">{title}</h2>\
        <p class="weui-msg__desc">{content}<a href="{url}">点击查看</a></p>\
    </div>\
    <div class="weui-msg__opr-area">\
        <p class="weui-btn-area">\
            <a href="{url}" class="weui-btn weui-btn_primary">确定</a>\
        </p>\
    </div>\
    <div class="weui-msg__extra-area">\
        <div class="weui-footer">\
            <p class="weui-footer__links">\
                <a href="{url}" class="weui-footer__link">查看详情</a>\
            </p>\
            <p class="weui-footer__text">Copyright &copy; coopui.js</p>\
        </div>\
    </div>\
</div>';
        this.showHtml = this.showHtml.replace(/{show_icon}/g, show_icon);
        this.showHtml = this.showHtml.replace(/{title}/g, title);
        this.showHtml = this.showHtml.replace(/{content}/g, content);
        this.showHtml = this.showHtml.replace(/{url}/g, url);
        $("body").html('');
        this.show();
    ...
    // 关闭窗口
    close: function(obj, relationobj) {
        if (arguments.length == 1) {
            $(obj).remove();
        } else {
            $(obj).parents('.' + relationobj).first().remove();
        }
        $(".weui-mask").remove();
        this.showHtml = '';
    },
    // 显示窗口
    show:function(){
        $('body').append(this.showHtml);
    },

    // 打开连接
    openurl: function (url, obj) {
        if (url) {
            location.href = url;
        } else {
            coop.close(obj);
    ...
    // 加载更多
    out_loadmore: function (msg, obj){
        var outMsg = ' <div class="weui-loadmore">\
                <i class="weui-loading"></i>\
                <span class="weui-loadmore__tips">{msg}</span>\
            </div>';
        outMsg = outMsg.replace(/{msg}/, msg);
        if (obj) {
            $(obj).html(outMsg);
        } else {
    ...
    // 加载不到数据
    out_loadnone: function (msg, obj) {
        var outMsg = '<div class="weui-loadmore weui-loadmore_line">\
            <span class="weui-loadmore__tips">{msg}</span>\
        </div>';
        outMsg = outMsg.replace(/{msg}/, msg);
        if (obj) {
